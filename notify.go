...
type NotificationHandlers struct {
	...
	// This will only be available when client is connected to a wallet
	// server such as hcashwallet.
	OnTicketsPurchased func(TxHash *chainhash.Hash, amount hcashutil.Amount)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	switch ntfn.Method {
	// OnBlockConnected
	case hcashjson.BlockConnectedNtfnMethod:
	...
	// OnBlockDisconnected
	case hcashjson.BlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case hcashjson.RelevantTxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case hcashjson.ReorganizationNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWinningTickets
	case hcashjson.WinningTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case hcashjson.SpentAndMissedTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnNewTickets
	case hcashjson.NewTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnStakeDifficulty
	case hcashjson.StakeDifficultyNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAccepted
	case hcashjson.TxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAcceptedVerbose
	case hcashjson.TxAcceptedVerboseNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBtcdConnected
	case hcashjson.BtcdConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		...
		if err != nil {
			log.Warnf("Received invalid hcashd connected "+
				"notification: %v", err)
			return
		}
	...
	// OnAccountBalance
	case hcashjson.AccountBalanceNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTicketPurchased:
	case hcashjson.TicketPurchasedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnVotesCreated:
	case hcashjson.VoteCreatedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case hcashjson.RevocationCreatedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case hcashjson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := hcashutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*hcashjson.TxRawResult,
	error) {
	...
	var rawTx hcashjson.TxRawResult
...
// parseBtcdConnectedNtfnParams parses out the connection status of hcashd
// and hcashwallet from the parameters of a btcdconnected notification.
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance hcashutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := hcashutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
func parseTicketPurchasedNtfnParams(params []json.RawMessage) (txHash *chainhash.Hash,
	amount hcashutil.Amount, err error) {
	...
	return thHash, hcashutil.Amount(amt), nil
}
...
func (c *Client) NotifyBlocksAsync() FutureNotifyBlocksResult {
	...
	cmd := hcashjson.NewNotifyBlocksCmd()
...
func (c *Client) NotifyWinningTicketsAsync() FutureNotifyWinningTicketsResult {
	...
	cmd := hcashjson.NewNotifyWinningTicketsCmd()
...
func (c *Client) NotifySpentAndMissedTicketsAsync() FutureNotifySpentAndMissedTicketsResult {
	...
	cmd := hcashjson.NewNotifySpentAndMissedTicketsCmd()
...
func (c *Client) NotifyNewTicketsAsync() FutureNotifyNewTicketsResult {
	...
	cmd := hcashjson.NewNotifyNewTicketsCmd()
...
func (c *Client) NotifyStakeDifficultyAsync() FutureNotifyStakeDifficultyResult {
	...
	cmd := hcashjson.NewNotifyStakeDifficultyCmd()
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := hcashjson.NewNotifyNewTransactionsCmd(&verbose)
	return c.sendCmd(cmd)
}
...
//
// NOTE: This is a hcashd extension and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []hcashutil.Address, outPoints []wire.OutPoint) error {
