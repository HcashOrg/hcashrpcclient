...
/*
Package hcashrpcclient implements a websocket-enabled Hypercash JSON-RPC client.

Overview

This client provides a robust and easy to use client for interfacing
with a Hypercash RPC server that uses a mostly btcd/bitcoin core
style Hypercash JSON-RPC API.  This client has been tested with hcashd
(https://github.com/HcashOrg/hcashd) and hcashwallet
(https://github.com/HcashOrg/hcashwallet).

In addition to the compatible standard HTTP POST JSON-RPC API, hcashd and
hcashwallet provide a websocket interface that is more efficient than the standard
...
hcashd or hcashwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by hcashd and
hcashwallet only uses a single connection that remains open and allows
...
All notifications provided by hcashd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by hcashd (and
hcashwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that hcashd intentionally separates the wallet
functionality into a separate process named hcashwallet.  This means if you are
connected to the hcashd RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, hcashwallet provides pass through treatment
...
detected by type asserting the error in a *hcashjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*hcashjson.RPCError); ok {
  		switch jerr.Code {
  		case hcashjson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - hcashdwebsockets
   Connects to a hcashd RPC server using TLS-secured websockets, registers for
   block connected and block disconnected notifications, and gets the current
   block count
 - hcashwalletwebsockets
   Connects to a hcashwallet RPC server using TLS-secured websockets, registers
