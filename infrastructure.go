...
// Client represents a Hypercash RPC client which allows easy access to the
// various RPC methods available on a Hypercash RPC server.  Each of the wrapper
...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *hcashjson.NotifyWinningTicketsCmd:
		c.ntfnState.notifyWinningTickets = true

	case *hcashjson.NotifySpentAndMissedTicketsCmd:
		c.ntfnState.notifySpentAndMissedTickets = true

	case *hcashjson.NotifyNewTicketsCmd:
		c.ntfnState.notifyNewTickets = true

	case *hcashjson.NotifyStakeDifficultyCmd:
		c.ntfnState.notifyStakeDifficulty = true

	case *hcashjson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *hcashjson.NotifyNewTransactionsCmd:
...
type (
	...
	rawResponse struct {
		...
		Error  *hcashjson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled hcashjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := hcashjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := hcashjson.MarshalCmd(id, cmd)
	if err != nil {
		return newFutureError(err)
	}
